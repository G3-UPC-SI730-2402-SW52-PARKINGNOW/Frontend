<template>
  <div class="owner-configuration-container">
    <div class="owner-configuration-row">
      <v-card class="owner-configuration-card">
        <h2>{{ $t('OWNER_CONFIGURATION.PRIVACY_POLICY.TITLE') }}</h2>
        <p>
        ...
        <button @click="toggleShowMore">{{ showMore ? $t('OWNER_CONFIGURATION.SHOW_LESS') : $t('OWNER_CONFIGURATION.SHOW_MORE') }}</button>
      </v-card>

      <v-card class="owner-configuration-card">
        <h2>{{ $t('OWNER_CONFIGURATION.LANGUAGE.TITLE') }}</h2>
        <v-select v-model="selectedLanguage" @change="changeLanguage">
          <v-list-item v-for="(lang, index) in languages" :key="index" :value="lang.code">
            {{ lang.name }}
          </v-list-item>
        </v-select>
        <v-btn @click="toggleLanguage" color="primary">
          {{ selectedLanguage === 'es' ? 'Cambiar a Inglés' : 'Change to Spanish' }}
        </v-btn>
      </v-card>
    </div>
    ...
    <div class="owner-configuration-row">
      <v-card class="owner-configuration-card">
        <h2>{{ $t('OWNER_CONFIGURATION.THEFT_REPORT.TITLE') }}</h2>
        <v-textarea label="{{ $t('OWNER_CONFIGURATION.THEFT_REPORT.DESCRIPTION') }}" v-model="theftDescription"></v-textarea>
        <v-btn @click="reportTheft">{{ $t('OWNER_CONFIGURATION.THEFT_REPORT.SUBMIT') }}</v-btn>
      </v-card>
    </div>

    <div class="owner-configuration-row">
      <v-card class="owner-configuration-card full-width">
        <h2>{{ $t('OWNER_CONFIGURATION.DELETE_ACCOUNT.TITLE') }}</h2>
